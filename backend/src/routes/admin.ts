...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const [last7DaysReservations, last7DaysRevenue, weeklyTrends] = await Promise.all([
        prisma.reservation.count({
          where: {
            date: {
              gte: last7Days,
        ...
        performance: {
          ...
          averageRevenuePerReservation:
            confirmedReservations > 0
              ? Number(totalRevenue._sum.estimatedAmount || 0) / confirmedReservations
              : 0,
        },
