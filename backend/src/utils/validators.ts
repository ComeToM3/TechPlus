...
export const reservationValidationSchema = Joi.object({
  ...
  partySize: Joi.number().integer().min(1).max(12).required().messages({
...
export const paymentValidationSchema = Joi.object({
  ...
  currency: Joi.string().length(3).uppercase().valid('USD', 'EUR', 'CAD').default('CAD').messages({
...
export const menuItemValidationSchema = Joi.object({
  ...
  allergens: Joi.array().items(Joi.string().max(50)).max(10).optional().messages({
...
export const tableValidationSchema = Joi.object({
  ...
  capacity: Joi.number().integer().min(1).max(12).required().messages({
...
export const restaurantValidationSchema = Joi.object({
  ...
  averagePricePerPerson: Joi.number().positive().precision(2).default(25.0).messages({
    'number.positive': 'Average price per person must be positive',
  }),
  minimumDepositAmount: Joi.number().positive().precision(2).default(10.0).messages({
    'number.positive': 'Minimum deposit amount must be positive',
  }),
  paymentThreshold: Joi.number().integer().min(1).max(20).default(6).messages({
...
export const reservationValidators = {
  ...
  getById: [param('id').isUUID().withMessage('Reservation ID must be a valid UUID')],
  delete: [param('id').isUUID().withMessage('Reservation ID must be a valid UUID')],
};
...
export const paymentValidators = {
  create: [
    ...
    body('reservationId').isUUID().withMessage('Reservation ID must be a valid UUID'),
  ],
  getById: [param('id').isUUID().withMessage('Payment ID must be a valid UUID')],
};
...
export const menuItemValidators = {
  create: [
    ...
    body('isAvailable').optional().isBoolean().withMessage('Availability must be a boolean value'),
  ],
  update: [
    param('id').isUUID().withMessage('Menu item ID must be a valid UUID'),
    body('name')
      .optional()
      .isLength({ min: 2, max: 100 })
      .withMessage('Menu item name must be between 2 and 100 characters'),
  ...
  getById: [param('id').isUUID().withMessage('Menu item ID must be a valid UUID')],
  delete: [param('id').isUUID().withMessage('Menu item ID must be a valid UUID')],
};
...
export const tableValidators = {
  create: [
    ...
    body('isActive').optional().isBoolean().withMessage('Active status must be a boolean value'),
  ],
  update: [
    param('id').isUUID().withMessage('Table ID must be a valid UUID'),
    body('number')
      .optional()
      .isInt({ min: 1 })
  ...
  getById: [param('id').isUUID().withMessage('Table ID must be a valid UUID')],
  delete: [param('id').isUUID().withMessage('Table ID must be a valid UUID')],
};
